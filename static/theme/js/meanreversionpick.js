...
$(function() {
    ...
    var url = 'https://www.quandl.com/api/v3/datatables/WIKI/PRICES.json?api_key=tVnPUGQgiPd2E3uTiGMy&qopts.columns=ticker&date=' + date;
    $.ajax({
      'url': yql_url,
      'data': {
...
var from_default = moment().subtract(7, 'years')
var from_max = moment().subtract(120, 'days')
...
$("#date_start").on("dp.change", function(e) {
  $('#date_end').data("DateTimePicker").minDate(e.date.add(120, 'days'));
});
$("#date_end").on("dp.change", function(e) {
  $('#date_start').data("DateTimePicker").maxDate(e.date.subtract(120, 'days'));
});
...
$("#date_start").click(function() {
  ...
  var yql_url = 'https://query.yahooapis.com/v1/public/yql';
  var url = 'https://www.quandl.com/api/v3/datatables/WIKI/PRICES.json?api_key=tVnPUGQgiPd2E3uTiGMy&qopts.columns=date&ticker=' + stock1;
  $.ajax({
    'url': yql_url,
    'data': {
      'q': 'SELECT * FROM json WHERE url="' + url + '"',
...
// jquery that dynamically updates the sma and lma available inputs

$('#sma').change(function() {

$('#lma option').removeAttr("disabled");
    var value = $(this).val();
    value = +value + 15;

$('#lma option').filter(function() {
...
$('#sma option').removeAttr("disabled");
    var value = $(this).val();
    value = +value - 15;

$('#sma option').filter(function() {
    return $(this).val() > value;
}).prop('disabled', true);
});
